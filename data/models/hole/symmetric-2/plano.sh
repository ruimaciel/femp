...
sed -n 's/^\t\+\[\t\([0-9]\+\),\[\t[0-9]\+\.[0-9]\+,\t0,\t0\t.*/{"node":\1, "dy":true, "dz":true},/p'  $file > ${file%.fem.json}-linhaX.txt
#sed -n 's/^\t\+\[\t\([0-9]\+\),\[\t0\+,\t[0-9]\+\.[0-9]\+,\t0\t.*/\1/p'  $file > ${file%.fem.json}-linhaY.txt
sed -n 's/^\t\+\[\t\([0-9]\+\),\[\t0\+,\t[0-9]\+\.[0-9]\+,\t0\t.*/{"node":\1, "dx":true, "dz":true},/p'  $file > ${file%.fem.json}-linhaY.txt

sed -n 's/^\t\+\[\t\([0-9]\+\),\[\t[0-9]\+\.[0-9]\+,\t0,\t0\t.*/\1,/p'  $file > ${file%.fem.json}-group-linhaX.txt
