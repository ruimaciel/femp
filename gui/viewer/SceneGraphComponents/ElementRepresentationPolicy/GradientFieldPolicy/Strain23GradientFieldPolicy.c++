...
void 
Strain23GradientFieldPolicy::calculateGradientValues (fem::Element &reference_element)
{
	...
	for(unsigned int coord = 0; coord < element->coordinates.size(); coord++)
	{
		...
		for(unsigned int node = 0; node < element->coordinates.size(); node++)
		{
			...
			// calculate \epsilon_{23} = dNdx_1*d1
			...
			m_gradient_value[coord] += dNdy*d.z() + dNdz*d.y(); // e_23 = du_2/dx_3 + du_3/dx_2 
		}
