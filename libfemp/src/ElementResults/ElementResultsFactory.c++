...
ElementResultsFactory::operator()(const fem::Element& reference_element)
{
    ...
    for (unsigned int coord = 0; coord < coordinates.size(); coord++) {
        ...
        results->von_mises[coord] = 0.7071067812 * sqrt((s.s11 - s.s22) * (s.s11 - s.s22) + (s.s22 - s.s33) * (s.s22 - s.s33) + (s.s33 - s.s22) * (s.s33 - s.s22) + 6 * (s.s12 * s.s12 + s.s23 * s.s23 + s.s13 * s.s13));
        // */
