...
enum json_error {
  ...
  JSON_WAITING_FOR_EOF, /*!< A complete JSON document tree was already finished
                           but needs to get to EOF. Other characters beyond
                           whitespaces produce errors */
...
typedef struct json_value {
  ...
  char *
      text; /*!< The text stored by the node. It stores UTF-8 strings and is
...
struct json_parsing_info {
  ...
  int string_length_limit_reached; /*!< flag informing if the string limit
                                      length defined by JSON_MAX_STRING_LENGTH
                                      was reached */
...
/**
The structure which holds the pointers to the functions that will be called by
the saxy parser whenever their evens are triggered
**/
...
@param document a reference to a json_t pointer, set to NULL, which will store
the parsed document
...
/**
Frees the memory appointed to the value fed as the parameter, as well as all the
child nodes
...
/**
Inserts a label:value pair into a parent node, as well as performs some document
tree integrity checks.
@param parent the parent node
@param text_label a char string which serves as the label in the label:value
pair
...
/**
Outputs a new UTF8 c-string which replaces all characters that must be escaped
with their respective escaped versions
...
document
@param root a reference to a pointer to a json_t type. The function allocates
memory to the passed pointer and sets up the value
...
/**
Function to perform a SAX-like parsing of any JSON document or document fragment
...
@return a pointer to the first label holding a text equal to text_label or NULL
if there is no such label or if object has no children
**/
