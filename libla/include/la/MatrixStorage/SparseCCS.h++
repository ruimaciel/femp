...
scalar SparseCCS<scalar>::value(const size_t row, const size_t column) {
	...
	if (row == data.row_index[i]) return data.values[i];

	if (row < data.row_index[i]) return 0;

	for (i = data.column_pointer[column]; i < data.column_pointer[column + 1]; i++) {
		if (data.row_index[i] < row) continue;

		if (data.row_index[i] == row) return data.values[i];
...
scalar& SparseCCS<scalar>::operator()(const size_t row, const size_t column) {
	...
	if (row == (size_t)data.row_index[data.column_pointer[column]]) return data.values[data.column_pointer[column]];
	...
	for (j = data.column_pointer[column]; j < data.column_pointer[column + 1]; j++) {
		if ((size_t)data.row_index[j] < row) continue;

		if ((size_t)data.row_index[j] == row) return data.values[j];
...
void SparseCCS<scalar>::resize(const size_t rows, const size_t columns) {
	...
	for (size_t j = 0; j <= columns; j++) data.column_pointer[j] = j;
}
