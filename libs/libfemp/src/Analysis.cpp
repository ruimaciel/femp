...
enum Analysis<Scalar>::Error Analysis<Scalar>::buildEquation(Model& model, const LoadPattern& lp, AnalysisResult& result, ProgressIndicatorStrategy& progress) {
	using namespace std;

	// perform sanity checks on the model
	if (model.getElementList().empty()) return ERR_NO_ELEMENTS;
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::generateGlobalStiffnessMatrix(Model& model, AnalysisResult& result, ProgressIndicatorStrategy& progress) {
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::generateGlobalDomainForceVector(Model& model, const LoadPattern& lp, AnalysisResult& result,
	...
	for (auto domain_load_pair : lp.domain_loads) {
		...
		for (size_t i = 0; i < el.getNodeAmount(); i++) {
			...
			if (dof == result.lm.end())
				...
				if (n != 0) result.equation.f(n - 1) += f_elem(3 * i);
				n = dof->second.get<1>();
				if (n != 0) result.equation.f(n - 1) += f_elem(3 * i + 1);
				n = dof->second.get<2>();
				if (n != 0) result.equation.f(n - 1) += f_elem(3 * i + 2);
			}
		}
		progress.markSectionIterationIncrement();
	}
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::generateGlobalSurfaceForceVector(Model& model, const LoadPattern& lp, AnalysisResult& result,
																				ProgressIndicatorStrategy& progress) {
	using namespace Eigen;
	...
	for (fem::SurfaceLoad* surface_load : lp.surface_loads) {
		...
		for (unsigned int i = 0; i < nodes.size(); i++) {
			...
			if (dof == result.lm.end())
				...
				size_t n = dof->second.get<0>();
				if (n != 0) result.equation.f(n - 1) += (Scalar)f_elem(3 * i);
				n = dof->second.get<1>();
				if (n != 0) result.equation.f(n - 1) += (Scalar)f_elem(3 * i + 1);
				n = dof->second.get<2>();
				if (n != 0) result.equation.f(n - 1) += (Scalar)f_elem(3 * i + 2);
			}
		}
		progress.markSectionIterationIncrement();
	}
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::generateGlobalPointForceVector(Model&, const LoadPattern& lp, AnalysisResult& result,
																			  ProgressIndicatorStrategy& progress) {
	using namespace Eigen;
	...
	for (std::map<size_t, fem::NodalLoad>::const_iterator nodal_load = lp.nodal_loads.begin(); nodal_load != lp.nodal_loads.end(); nodal_load++) {
		...
		// set the nodal loads
		if (result.lm[n].template get<0>() != 0) result.equation.f(result.lm[n].template get<0>() - 1) += force.x();
		if (result.lm[n].template get<1>() != 0) result.equation.f(result.lm[n].template get<1>() - 1) += force.y();
		if (result.lm[n].template get<2>() != 0) result.equation.f(result.lm[n].template get<2>() - 1) += force.z();
...
template <typename Scalar>
std::map<size_t, Node> Analysis<Scalar>::displacementsMap(AnalysisResult& result) {
	using namespace std;
	...
	for (map<size_t, boost::tuple<size_t, size_t, size_t>>::const_iterator i = result.lm.begin(); i != result.lm.end(); i++) {
		...
		if ((i->second.get<0>() == 0) && (i->second.get<1>() == 0) && (i->second.get<2>() == 0)) continue;

		// assign the displacements
		if (i->second.get<0>() != 0) node.data[0] = result.equation.d(n++);
		if (i->second.get<1>() != 0) node.data[1] = result.equation.d(n++);
		if (i->second.get<2>() != 0) node.data[2] = result.equation.d(n++);
...
template <typename Scalar>
void Analysis<Scalar>::generateDisplacementsMap(Model& model, AnalysisResult& result) {
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::recoverValues(Model& model, AnalysisResult& result) {
...
template <typename Scalar>
enum Analysis<Scalar>::Error Analysis<Scalar>::calculateStrainEnergy(Model&, AnalysisResult& result) {
...
template <typename Scalar>
void Analysis<Scalar>::makeLocationMatrix(Model& model, AnalysisResult& result) {
...
template <typename Scalar>
inline void Analysis<Scalar>::addElementaryStiffnessToGlobal(const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>& k_elem,
															 std::map<size_t, boost::tuple<size_t, size_t, size_t>>& lm, Element& element,
