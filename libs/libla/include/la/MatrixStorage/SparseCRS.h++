...
scalar SparseCRS<scalar>::value(const size_t row, const size_t column) {
	...
	if (column == data.column_index[i]) return data.values[i];

	if (column < data.column_index[i]) return 0;

	for (i = data.row_pointer[row]; i < data.row_pointer[row + 1]; i++) {
		if (data.column_index[i] < column) continue;

		if (data.column_index[i] == column) return data.values[i];
...
scalar& SparseCRS<scalar>::operator()(const size_t row, const size_t column) {
	...
	if (column == data.column_index[data.row_pointer[row]]) return data.values[data.row_pointer[row]];
	...
	for (j = data.row_pointer[row]; j < data.row_pointer[row + 1]; j++) {
		if (data.column_index[j] < column) continue;

		if (data.column_index[j] == column) return data.values[j];
...
void SparseCRS<scalar>::resize(const size_t rows, const size_t columns) {
	...
	for (size_t i = 0; i < columns; i++) data.column_index[i] = i;

	for (size_t j = 0; j < rows; j++) data.row_pointer[j] = j;
