...
#include "InputStates/Input.hpp"  // for the state pattern that handles user input
#include "SceneGraphComponents/Operations/SelectFrustumInclusionOperation.hpp"
#include "SceneGraphComponents/Operations/SelectRayIntersectionOperation.hpp"
#include "SceneGraphComponents/Operations/ToggleRenderOperation.hpp"
#include "SceneGraphComponents/Operations/ToggleSelectionOperation.hpp"
#include "ViewportStates/ViewportState.hpp"	 // for the vieport's state pattern base class

BaseViewport::BaseViewport(fem::Project& project, QWidget* parent) : QOpenGLWidget(parent), m_project(project) {
...
void BaseViewport::resizeGL(int width, int height) {
	...
	glOrtho(-(width * 2) / (viewport_data.aspect_ratio * pow(2, viewport_data.zoom)), (width * 2) / (viewport_data.aspect_ratio * pow(2, viewport_data.zoom)),
			-height * 2 / (viewport_data.aspect_ratio * pow(2, viewport_data.zoom)), +height * 2 / (viewport_data.aspect_ratio * pow(2, viewport_data.zoom)),
			0.1, 1000.0);
...
void BaseViewport::setTrianglesVisible(bool const state) {
	...
	if (state == false) {
		qCritical() << "BaseViewport::setTrianglesVisible(bool const state): "
					   "this->state is nullptr";
		return;
	}
...
void BaseViewport::selectObjectsFromFrustum(std::array<fem::Point3D, 4> const& near, std::array<fem::Point3D, 4> const& far) {
	Selection selection;
...
void BaseViewport::showAll() {
	...
	Selection selection;
	Operation::ToggleRenderOperation op(selection, false);	// turns on all elements which aren't selected, which
															// in this case means all elements
	this->state->runSceneGraphOperation(op);
}
