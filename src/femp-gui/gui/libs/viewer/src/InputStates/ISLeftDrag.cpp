...
#include "../ViewportStates/ViewportState.hpp"
#include "Input.hpp"
...
void LeftDrag::leftRelease(BaseViewport* viewport, QMouseEvent* event, Input* input) {
	...
	input->changeState(&input->m_is_start);
	{
		...
		auto add = [&near, &far, &vport, &viewport](fem::Point3D& pos, int const i) {
			gluUnProject(pos.x(), vport[3] - pos.y(), 0, viewport->viewport_data.modelview, viewport->viewport_data.projection, vport, &near[i].data[0],
						 &near[i].data[1], &near[i].data[2]);
			gluUnProject(pos.x(), vport[3] - pos.y(), 1, viewport->viewport_data.modelview, viewport->viewport_data.projection, vport, &far[i].data[0],
						 &far[i].data[1], &far[i].data[2]);
		};
