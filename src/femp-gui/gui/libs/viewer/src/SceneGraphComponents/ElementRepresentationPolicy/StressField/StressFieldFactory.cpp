#include "StressFieldFactory.hpp"
...
StressFieldRepresentation StressFieldFactory::operator()(fem::Element const& element) {
	assert(m_model != nullptr);

	StressFieldRepresentation representation;

	fem::BaseElement* e = nullptr;
	std::vector<fem::Point3D> local_points;	 // list of local_points coordinates where the tensors will
											 // be evaluated
	...
	// helper
	// auto di = [&m_diameter, &m_model, &element](float const scale,
	// std::initializer_list< std::pair<fem::node_ref_t, fem::node_ref_t> > list)
	auto di = [&](float const scale, std::initializer_list<std::pair<fem::node_ref_t, fem::node_ref_t>> list) {
		for (auto a : list) {
			...
			if (scale * scale * dot_product(d, d) < (*m_diameter) * (*m_diameter)) *m_diameter = scale * d.norm();
		}
	};
	...
	switch (element.getType()) {
		...
		case fem::Element::FE_HEXAHEDRON20:
			e = &m_hexahedron20;
			local_points = {{-0.57735, -0.57735, -0.57735}, {0.57735, -0.57735, -0.57735}, {0.57735, 0.57735, -0.57735}, {-0.57735, 0.57735, -0.57735},
		...
		case fem::Element::FE_PRISM15:
			e = &m_prism15;
			local_points = {{1.0 / 3, 1.0 / 3, -0.57735}, {1.0 / 3, 1.0 / 3, 0.57735}};
