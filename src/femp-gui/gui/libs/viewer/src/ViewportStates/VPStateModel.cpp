...
VPStateModel::VPStateModel() : ViewportState() {
...
void VPStateModel::populateScenegraph(BaseViewport* viewport) {
	...
	for (auto node_restrictions_pair : femp_model.getNodeRestrictions()) {
		...
		auto component = std::shared_ptr<SceneGraphComponent>(
			new SGC::NodeRestrictions(node_restrictions_pair.first, node_restrictions_pair.first, node_restrictions_pair.second, &this->m_no_displacements));
