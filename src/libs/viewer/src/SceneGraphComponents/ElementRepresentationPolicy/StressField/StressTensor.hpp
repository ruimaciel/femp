#ifndef FEMP_VIEWER_STRESSTENSOR_HPP
#define FEMP_VIEWER_STRESSTENSOR_HPP
...
class StressTensor {
	public:
	fem::Point3D m_position;	  // the position in space where the stress tensor
								  // will be rendered
	...
	void render(float const& diameter, float const& max, float const& min, bool const& render_positive, bool const& render_negative,
				ViewportColors const& colors);

	protected:
	/**
	 * Sets the color value by interpolating a float between the maximum and
