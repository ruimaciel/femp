#include "SelectFrustumInclusionOperation.hpp"

#include <Project.hpp>

namespace Operation {

SelectFrustumInclusionOperation::SelectFrustumInclusionOperation(Selection& selection, std::array<fem::Point3D, 4> const& near,
																 std::array<fem::Point3D, 4> const& far)
...
void SelectFrustumInclusionOperation::visit(SGC::Node& node) {
	if (node.render) {
		for (int i = 0; i < 4; i++) {
			if (dot_product(m_normal[i], node.pos() - m_near[i]) < 0) return;
		}
...
void SelectFrustumInclusionOperation::calculateNormalVectors(std::array<fem::Point3D, 4> const& near, std::array<fem::Point3D, 4> const& far) {
	auto calc = [](fem::Point3D const& p0, fem::Point3D const& p1, fem::Point3D const& p2) -> fem::Point3D {
		// calculates the normal vector which is perpendicular to the plane defined
		// by (p1-p0),(p2-p0)
