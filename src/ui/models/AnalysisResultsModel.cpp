#include "AnalysisResultsModel.hpp"
...
AnalysisResultsModel::AnalysisResultsModel(fem::Project& project, QObject* parent) : QAbstractTableModel(parent), m_project(project) {
...
QVariant AnalysisResultsModel::headerData(int section, Qt::Orientation orientation, int role) const {
...
QVariant AnalysisResultsModel::data(const QModelIndex& index, int role) const {
	...
	for (i = m_lineMap.rbegin(); i->first > index.row(); i++)
		;
	if (i == m_lineMap.rend()) return QString("out of bounds");
